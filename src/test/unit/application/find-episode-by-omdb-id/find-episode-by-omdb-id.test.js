...
const FindEpisodeByOmdbIdResponse = require('../../../../application/find-episode-by-omdb-id/find-episode-by-omdb-id-response');

describe('Find episode by omdb id', () => {
	const episodeRepositoryMock = {
		findByOmdbId: jest.fn()
	};
	const id = 619;
	...
	beforeEach(() => {
		findEpisodeByOmdbIdMock = new FindEpisodeByOmdbId({
			episodeRepository: episodeRepositoryMock
		});
	});

	test('Should find movie', async () => {
		const foundEpisode = new Episode(episodeMock);
		episodeRepositoryMock.findByOmdbId.mockReturnValue(foundEpisode);

		const commandMock = new FindEpisodeByOmdbIdCommand({omdb_id: id})
		const response = await findEpisodeByOmdbIdMock.find(commandMock);

		expect(episodeRepositoryMock.findByOmdbId).toHaveBeenCalledWith(id);
		expect(episodeRepositoryMock.findByOmdbId).toHaveBeenCalledTimes(1);

		expect(response).toBeInstanceOf(FindEpisodeByOmdbIdResponse);
	});
});