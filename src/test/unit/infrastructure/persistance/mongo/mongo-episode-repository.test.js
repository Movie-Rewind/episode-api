...
describe('Episode repository', () => {
	...
	const episodeDocumentParserMock = {toDocument: jest.fn(), toDomain: jest.fn()};
	const episodeMock = {
		"air_date": "2011-04-17",
		"crew": [
			{
	...
	beforeEach(() => {
		mongoEpisodeRepositoryMock = new MongoEpisodeRepository({
			mongoDbHandler: mongoDbHandlerMock,
			episodeDocumentParser: episodeDocumentParserMock
		});
	});

	it('Save function', async () => {
		const episodeDomain = new Episode(episodeMock);
		...
		episodeDocumentParserMock.toDocument.mockReturnValue(episodeMock);

		await mongoEpisodeRepositoryMock.save(episodeDomain);
		...
		expect(dbInstanceMock.insertOne).toHaveBeenCalledWith(episodeMock);
	});
